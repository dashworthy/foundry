...
/**
 * Laravel core ships no `make:action`, so unlike a generator that overrides an
 ...
 */
abstract class ActionMakeCommand extends GeneratorCommand
{
    protected $name = 'make:action';

    protected $description = 'Create a new action class and its data';

    protected $type = 'Action';

    /**
     * An action without a Data object does not run, so the pair is always
     * scaffolded together.
     */
    ...
    /**
     * Fill in {{ dataNamespace }} so the generated action's `use` statement for
     ...
     * rather than inferring it from this action's own namespace, so the two
     * cannot drift. `protected` so the domain-aware override in
     * `dashworthy/domains` (which knows the domain `subdirectories` map and can
     * compute the answer authoritatively) can replace it wholesale instead of
     * this class having to guess at domain placement.
     */
